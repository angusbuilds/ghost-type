...
cd ghost-type
node --test          # 334 tests, all offline — spends no tokens
```
...
334 tests, all offline (`node --test`) · independently audited by Codex (gpt-5.6-sol, xhigh) across twenty-eight rounds — every finding fixed
```
