...
cd ghost-type
node --test          # 247 tests, all offline — spends no tokens
```
...
247 tests, all offline (`node --test`) · independently audited by Codex (gpt-5.6-sol, xhigh) across eighteen rounds — every finding fixed
```
