...
cd ghost-type
node --test          # 228 tests, all offline — spends no tokens
```
...
228 tests, all offline (`node --test`) · independently audited by Codex (gpt-5.6-sol, xhigh) across five rounds — every finding fixed
```
