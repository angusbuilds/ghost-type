...
cd ghost-type
node --test          # 268 tests, all offline — spends no tokens
```
...
268 tests, all offline (`node --test`) · independently audited by Codex (gpt-5.6-sol, xhigh) across twenty-one rounds — every finding fixed
```
