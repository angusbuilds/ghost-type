...
cd ghost-type
node --test          # 264 tests, all offline — spends no tokens
```
...
264 tests, all offline (`node --test`) · independently audited by Codex (gpt-5.6-sol, xhigh) across twenty rounds — every finding fixed
```
