...
cd ghost-type
node --test          # 372 tests, all offline — spends no tokens
```
...
372 tests, all offline (`node --test`) · independently audited by Codex (gpt-5.6-sol, xhigh) and by a parallel multi-agent sweep — every finding fixed
```
